...
- 有向无环图的拓扑排序/逆拓扑排序：
  - 拓扑排序方法：
    - aov网，activity on vertices以顶点为活动的网
...
- 最短路径算法：
  - 基本概念：
    - aoe网，activity on edges以边为活动的网
    - aoe网中只有一个入度为0的顶点：源点，和一个出度为0的顶点：汇点
    - 关键路径：从源点到汇点长度最大的路径
    - 事件v<sub>k</sub>的最早发生时间ve(k)：决定了所有从vk开始的活动能开工的最早时间
    - 事件v<sub>k</sub>的最迟发生时间vl(k)：在不推迟工程完成的前提下，事件最迟发生的时间
    - 活动a<sub>i</sub>的最早开始时间e(i)：该活动代表的弧的起点代表的事件的最早发生时间
    - 活动a<sub>i</sub>的最迟开始时间l(i)：该活动代表的弧的终点代表的事件的最迟发生时间与该活动所需时间之差
    - 活动a<sub>i</sub>的时间余量d(i)=l(i)-e(i)：不推迟工程完成的前提下，活动能拖延的最多时间
    - 关键活动：时间余量为0的活动
    - 关键路径：关键活动组成的路径
  - 求关键路径步骤：
    1. 求所有事件的最早发生时间
    ...
    4. 求所有活动的最迟发生时间
    5. 求所有活动的时间余量
    - 注意：源点的最早发生时间为0，汇点的最迟发生时间和最早发生时间相等
  - 关键路径性质：
    - 缩短/加长关键路径的长度将会减少/增加整个工程的工期
    - 当有多个关键路径时只有缩短所有关键路径的长度才会减少工期